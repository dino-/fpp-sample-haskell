...
defaultOptions = Options
   ...
   , optHelp = False
   , optLink = False
   , optPrefix = "/cfs/devA/projects/opt"
   , optRsrcCpVerbose = False
   , optInstType = Bundle
   , optVersion = False
   }
...
main = do
   ...
   when (optDelete opts && appDirExists) $ do
      putStrLn $ "Removing existing directories under " ++ (appDir dirs)
      removeDirectoryRecursive $ appDir dirs </> "bin"
      removeDirectoryRecursive $ appDir dirs </> "doc"
      removeDirectoryRecursive $ appDir dirs </> "resources"

   -- Clean before building
   when (optClean opts) $ system "stack clean" >> return ()

   -- Copy the binaries
   -- NOTE For some reason this directory has to exist before we
   -- `stack install`, maybe a quirk of Lustre or something. Not
   -- normally needed.
